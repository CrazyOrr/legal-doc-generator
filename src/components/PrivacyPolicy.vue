<template>
  <div class="container py-2">
    <b-form v-if="step === 0" @reset.prevent="onReset" novalidate>
      <b-form-group
          label="产品名称："
          label-for="product-name-input"
          description="您的APP或者网站的名称。"
          :invalid-feedback="productNameInvalidFeedback"
          :state="isProductNameValid"
      >
        <b-form-input
      ...
      <b-form-group
          ...
          description="您的电话或电子邮件地址信息。"
          :invalid-feedback="contactInfoInvalidFeedback"
          :state="isContactInfoValid"
      >
        <b-form-input
      ...
      <b-button type="button" variant="primary" :disabled="!isProductNameValid || !isContactInfoValid" @click="nextStep">下一步</b-button>
    </b-form>

    <div v-if="step === 1">
      <div id="result" class="d-flex flex-row">
        <div id="preview" class="d-flex flex-column">
          <label class="px-3 py-2 mb-0" for="preview-content">预览</label>
          <div id="preview-content" class="border p-2" v-html="html"></div>
        </div>
        <div id="src" class="d-flex flex-column">
          <div id="src-top-bar" class="d-flex justify-content-between">
            <b-tabs @input="onTabInput">
              <b-tab title="HTML" active>
              </b-tab>
              <b-tab title="Markdown">
              </b-tab>
            </b-tabs>
            <b-button id="copy-btn" type="button" variant="primary" @click="copy">复制</b-button>
            <b-tooltip
                target="copy-btn"
                :variant="copyTooltipVariant"
                placement="bottom"
                :show="copyTooltipText !== null"
                triggers="manual">
              {{copyTooltipText}}
            </b-tooltip>
          </div>
      ...
      <div class="mt-3 d-flex justify-content-between align-items-center">
        <b-button class="flex-shrink-0" type="button" variant="outline-primary" @click="previousStep">上一步</b-button>
        <span class="text-muted small ml-3">免责声明：隐私政策模板来源于互联网，我们不保证内容的正确性。通过使用本站内容随之而来的风险与本站无关。</span>
      </div>
    </div>
  </div>
</template>
...
<div>
  <h2>引言</h2>
  <p>
    ${this.productName}重视用户的隐私。您在使用我们的服务时，我们可能会收集和使用您的相关信息。我们希望通过本《隐私政策》向您说明，在使用我们的服务时，我们如何收集、使用、储存和分享这些信息，以及我们为您提供的访问、更新、控制和保护这些信息的方式。本《隐私政策》与您所使用的${this.productName}服务息息相关，希望您仔细阅读，在需要时，按照本《隐私政策》的指引，作出您认为适当的选择。本《隐私政策》中涉及的相关技术词汇，我们尽量以简明扼要的表述，并提供进一步说明的链接，以便您的理解。</p>
  <p><strong>您使用或继续使用我们的服务，即意味着同意我们按照本《隐私政策》收集、使用、储存和分享您的相关信息。</strong></p>
  <p>如对本《隐私政策》或相关事宜有任何问题，请通过${this.contactInfo}与我们联系。</p>
</div>
...
<div>
  ...
  <p><strong>因此，请您谨慎考虑通过我们的服务上传、发布和交流的信息内容。</strong>在一些情况下，您可通过我们某些服务的隐私设定来控制有权浏览您共享信息的用户范围。如要求从我们的服务中删除您的相关信息，请通过该等特别服务条款提供的方式操作。
  </p>
</div>
...
<div>
  ...
  <ul>
    <li>
      我们的商品或服务，我们的关联公司和合作伙伴的商品或服务，包括即时通讯服务、网上媒体服务、互动娱乐服务、社交网络服务、付款服务、互联网搜索服务、位置和地图服务、应用软件和服务、数据管理软件和服务、网上广告服务、互联网金融，以及其他社交媒体、娱乐、电子商务、资讯和通讯软件或服务（统称“<strong>互联网服务</strong>”）；
    </li>
    <li>
...
`
    ...
    src() {
      return this.tabIndex === 0 ? this.html : this.markdown
    }
  },
  created() {
  ...
  methods: {
    ...
    copy() {
      let showCopyResult = (isSuccessful, payload) => {
        if (isSuccessful) {
          console.debug('copied:')
        } else {
          console.error('failed to copy:', payload)
        }
        this.copyTooltipVariant = isSuccessful ? 'success' : 'danger'
        this.copyTooltipText = isSuccessful ? '复制成功' : '复制失败'
        if (this.timeoutId) {
          clearTimeout(this.timeoutId)
        }
        this.timeoutId = setTimeout(() => {
          this.copyTooltipText = null
        }, 2000)
      }
      let src = this.src
      if (navigator.clipboard) {
        navigator.clipboard.writeText(src)
            .then(() => {
              showCopyResult(true, src)
            }, (err) => {
              showCopyResult(false, err)
            })
      } else {
        this.$refs.textarea.select()
        let isSuccessful = document.execCommand('copy')
        showCopyResult(isSuccessful)
      }
    }
  }
}
